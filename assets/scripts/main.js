...
function updateTotals() {
  const tax = 15;
  const subtotalValue = productsListed.reduce(
  ...
  if (code.toLowerCase() === DISCOUNT_CODE) {
    if (!promoApplied) {
      ...
      promoError.textContent =
        "Promo code applied successfully (75% discount)!";
...
function populateReviewSection() {
  ...
  productsListed.forEach((p) => {
    summaryHtml += `<p>${p.productName} (x${p.quantity}): $${(
      p.price * p.quantity
    ).toFixed(2)}</p>`;
  });
  ...
  summaryHtml += `
    ...
    `;
  if (promoApplied) {
    summaryHtml += `<p style="color: green;"><strong>Discount (75%):</strong> -$${discountAmount.toFixed(
      2
    )}</p>`;
  }
  summaryHtml += `
            ...
            <p><strong>Order Total:</strong> <span style="font-size: 20px; color: #435274; font-weight: bold;">$${totalPriceValue.toFixed(
              2
            )}</span></p>
        </div>
    `;
  ...
  if (billingAndShipping.checked) {
    ...
    const bCountryText =
      billingData[3].options[billingData[3].selectedIndex].text;
    ...
    const sCountryText =
      shippingData[3].options[shippingData[3].selectedIndex].text;
  ...
  const obscuredCard = cardNumber.value.replace(
    /(\d{4}\s){3}/,
    "XXXX XXXX XXXX "
  );
...
let curSection = 0; // 0-indexed: 0-Checkout, 1-Personal, 2-Address, 3-Payment, 4-Review, 5-loading, 6-Confirmation
...
function setCurSection(value) {
  curSection = value;
  if (curSection === 5) {
    // HIDE BUTTONS IMMEDIATELY on entering the loading screen (5)
    nextBtn.classList.add("d-none");
    prevBtn.classList.add("d-none");

    setTimeout(() => {
      // GENERATE RESULT AND TRANSITION TO CONFIRMATION (6) AFTER DELAY
      ...
      // Show/Update buttons for the Confirmation screen (6)
...
const updateButtonVisibility = () => {
  ...
  if (curSection === 5) {
    // Buttons are already explicitly hidden in setCurSection, but this ensures they stay hidden.
    prevBtn.classList.add("d-none");
    nextBtn.classList.add("d-none");
  }

  if (curSection === mainSections.length - 1) {
    // Final confirmation page (index 6)
    nextBtn.classList.add("d-none"); // HIDE next button on confirmation page
    if (randomNumber === 0) {
      // If FAIL (0) - requirement 2
      prevBtn.classList.remove("d-none"); // SHOW back button (to allow retry/review)
    } else {
      // If SUCCESS (1) - requirement 2
      prevBtn.classList.add("d-none"); // HIDE back button
    }
  } else {
    // Ensure buttons are visible for normal navigation
    if (curSection !== 5) {
...
nextBtn.onclick = () => {
  ...
  if (curSection === mainSections.length - 3) {
    // When on Review (4)
    // Transition from Review (4) to Loading (5)
    mainSections[curSection].classList.add("d-none");
    setCurSection(curSection + 1); // curSection becomes 5. The rest is handled in setCurSection's setTimeout
    mainSections[curSection].classList.remove("d-none");

    // The result logic, transition to 6, and button updates are now handled in the setCurSection setTimeout
    // Only update progress here to show the loading screen is active
    updateProgress(curSection + 1);
    return;
  }

  if (curSection < mainSections.length - 1) {
    // Stop before the last confirmation section
    mainSections[curSection].classList.add("d-none");
...
prevBtn.onclick = () => {
  // If currently on the Confirmation screen (index 6, mainSections.length - 1)
  if (curSection === mainSections.length - 1) {
    // Go back two steps to Review screen (index 4)
    mainSections[curSection].classList.add("d-none");
    setCurSection(curSection - 2); // Sets curSection to 4
    mainSections[curSection].classList.remove("d-none");
  } else if (curSection > 0) {
    // For all other sections (1 to 4), go back one step
    mainSections[curSection].classList.add("d-none");
    setCurSection(curSection - 1);
    mainSections[curSection].classList.remove("d-none");
  } else {
    return; // Already on step 0, do nothing
  }

  // Update UI (Progress is based on current section index + 1)
  updateButtonVisibility();
  updateBtnLabels();
  updateProgress(curSection + 1);
};
